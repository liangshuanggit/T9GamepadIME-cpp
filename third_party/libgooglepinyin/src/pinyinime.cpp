...
extern "C" {
  ...
  int im_validate_splstr(const char* sps_buf, size_t sps_len) {
    if (NULL == sps_buf || 0 == sps_len)
      return 0;

    SpellingParser parser;
    uint16 spl_idx[256];
    uint16 start_pos[257];
    bool last_is_pre = false;

    uint16 idx_num = parser.splstr_to_idxs(sps_buf, (uint16)sps_len,
                                            spl_idx, start_pos, 256,
                                            last_is_pre);

    if (0 == idx_num) {
      // No complete syllables found. Check if the string could be a valid
      // prefix (all characters are valid spelling characters that the parser
      // could have started consuming).
      for (size_t i = 0; i < sps_len; ++i) {
        if (!parser.is_valid_to_parse(sps_buf[i]))
          return 0;
      }
      return 1;  // PREFIX
    }
    ...
    if (start_pos[idx_num] != (uint16)sps_len)
      return 1;  // PREFIX (partial syllable at the end)

    // All characters consumed and the last syllable was successfully recorded
    // (start_pos[idx_num] == sps_len means if_valid_id_update succeeded for
    // the last syllable, i.e. it is a complete syllable).
    // Note: last_is_pre from splstr_to_idxs is unreliable (always true when
    // the string doesn't end with a splitter), so we don't use it here.
    return 2;  // COMPLETE
  }
